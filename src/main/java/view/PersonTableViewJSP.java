...
public class PersonTableViewJSP extends HttpServlet {
    private void fillTableData( HttpServletRequest req, HttpServletResponse resp )
        ...
        req.getRequestDispatcher( "/jsp/ShowTable-Pesron.jsp" ).forward( req, resp );
    }
    /**
     * this method is searching for a list of column name and code in person class
     * @param req
     * @return 
     */
    private List<?> extractTableData(HttpServletRequest req) {
        ...
        List<Person> list;
        /**
         * this if is searching for not empty record, so when it find the name that we are searching for, it will get all the similar the search work key
         */
                if( search != null ){
            list = logic.search( search );
        } else {
            list = logic.getAll(); //  it will get all the key word that we search for
        } 
        if( list == null || list.isEmpty() ){ // if we don't find the key word that we looking for it will return nothoing.
    ...
    /**
     * this is the append method wich add new record to the array list.
     * @param <T>
     * @param list
     * @param toArray
     * @return newlist
    ...
    /**
     * the toStringMap method is the representation of the consists of a list that have key and value
